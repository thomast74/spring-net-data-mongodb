...
namespace Spring.Data.MongoDb.Core
{
    ...
    public class MongoDbUtils
    {
        ...
        {
            return DoGetDatabase(mongo, databaseName, null, true);
        }
        ...
        public static MongoDatabase GetDatabase(MongoServer mongo, string databaseName, MongoCredentials credentials)
        {
            AssertUtils.ArgumentNotNull(mongo, "mongo");
            AssertUtils.ArgumentHasText(databaseName, "databaseName");
            AssertUtils.ArgumentNotNull(credentials, "credentials");

            return DoGetDatabase(mongo, databaseName, credentials, true);
        }

        private static MongoDatabase DoGetDatabase(MongoServer mongo, string databaseName, MongoCredentials credentials,
                                                   bool allowCreate)
        {
            var dbHolder = (DbHolder) TransactionSynchronizationManager.GetResource(mongo);
            ...
            var newDatabase = credentials != null
                                  ? mongo.GetDatabase(databaseName, credentials)
            ...
            if (TransactionSynchronizationManager.SynchronizationActive)
            {
                ...
                if (holderToUse == null)
                {
                    holderToUse = new DbHolder(databaseName, newDatabase);
                }
                else
                {
